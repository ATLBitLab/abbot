...
from lib.abbot.env import AbbotEnv


class AbbotConfig(AbbotEnv):
    ORG_CONFIG = json_loader(CONFIG_JSON_FILEPATH)
    ...
    BOT_INTRO = f"Your name is {BOT_NAME}, which is short for {BOT_NAME_MEANING}, your telegram handle is {BOT_TG_HANDLE}. You answer to Abbot. You are part of {ORG_NAME} - {ORG_DESCRIPTION} and you are an expert in all things Atlanta, ATL BitLab, Bitcoin and Lightning Network. {BOT_DIRECTIVES}."
    ...
    def __init__(self, dev_mode: bool | None = None, name: str | None = None, handle: str | None = None):
        self.BOT_NAME = name or self.BOT_NAME
        self.BOT_TG_HANDLE = handle or self.BOT_TG_HANDLE
        ENV: AbbotEnv = AbbotEnv()
        self.BOT_ENV.to_dict()
        if dev_mode:
            self.NAME = f"t{name or self.NAME}"
            self.BOT_TG_HANDLE = f"test_{handle or self.BOT_TG_HANDLE}"
            self.TG_TOKEN = AbbotEnv.TEST_TG_TOKEN
    ...
    def get_bot_env(self):
        return dict(NAME=self.NAME, TG_HANDLE=self.TG_HANDLE, TG_TOKEN=self.TG_TOKEN)

    def get_bot_response(self, response_type: str, index: int = None) -> str:
        response_list = try_get(self.RESPONSES, response_type)
        index = randrange(len(response_list)) if not index else index
        return try_get(response_list, index)


def setup_abbot_env_config(dev_mode: bool):
    bot_env: AbbotEnv = AbbotEnv(dev_mode)
    abbot_config: AbbotConfig = AbbotConfig(bot_env)
    return abbot_config
